...
const ProfileDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
      <header className="bg-white border-b border-gray-200 shadow-sm px-4 lg:px-10 py-8 md:py-10">
        <div className="mx-auto container flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 text-center md:text-left">
            ...
            <div>
              ...
              <p className="text-gray-500 font-medium text-sm md:text-base">{profileData.email}</p>
            </div>
          </div>
          <button
      ...
      {/* Navigation */}
      <div className="mx-auto container px-4 lg:px-10 mt-8">
        <nav className="flex items-center overflow-x-auto gap-1 md:gap-2 bg-gray-100 p-1.5 rounded-2xl md:rounded-full no-scrollbar">
          {[
            ...
            { id: "orders", icon: <ShoppingBag size={18} />, label: `Orders (${orders?.length || 0})` },
      ...
      <main className="mx-auto container px-4 lg:px-10 mt-8">
        {activeTab === "profile" && (
          <section className="bg-white rounded-[24px] md:rounded-[32px] border border-gray-200 px-6 py-8 shadow-sm">
            ...
            <div className="space-y-6">
              {orders && orders.length > 0 ? (
                [...orders].reverse().map((order) => (
                  <div key={order.id} className="py-6 px-2 border border-gray-100 rounded-[24px] bg-gray-50 hover:border-black/10 transition-all shadow-sm">
                    ...
                    <div className="border-t border-gray-200 pt-4 mt-4">
                      <p className="font-bold text-sm mb-3 text-gray-500 uppercase tracking-tight">Items</p>
                          ...
                          <div key={idx} className="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-100">
                            <div className="flex items-center gap-3">
                               ...
                               <div>
                                 ...
                                 <p className="text-[10px] text-gray-500 uppercase">{item.size} | {item.color}</p>
                               </div>
                            </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-gray-200 pt-4 mt-4">
                      <div>
                        ...
                        <p className="text-xs font-bold text-black truncate">{order.address}</p>
                      </div>
                      <div className="sm:text-center">
                        <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Method</p>
                        <p className="text-xs font-bold text-black uppercase">{order.paymentMethod}</p>
                      </div>
                      <div className="sm:text-right">
                        <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Grand Total</p>
                ...
                <div className="text-center py-20 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                  ...
                  <p className="font-bold text-gray-400">No orders found in your history.</p>
                </div>
              )}
            </div>
          </section>
        ...
        {/* Wishlist & Settings */}
        {activeTab === "wishlist" && (
          <section className="bg-white rounded-[24px] md:rounded-[32px] border border-gray-200 py-12 shadow-sm text-center">
...
const InfoCard = ({ icon, label, value }) => (
  <div className="flex items-center gap-3 md:gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-black transition-all">
    <div className="p-3 bg-white rounded-xl shadow-sm text-gray-400 shrink-0">{icon}</div>
