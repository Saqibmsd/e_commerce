...
export default function CasualPage() {
  ...
  const [appliedFilters, setAppliedFilters] = useState({
    category: "", color: "", size: "", style: "", price: 500,
  });

  const [currentPage, setCurrentPage] = useState(1);
  const productsPerPage = 6;
  const [openSections, setOpenSections] = useState({
    price: true, colors: true, size: true, style: true,
  });
  ...
  const colors = [
    ...
    "bg-cyan-400", "bg-blue-700", "bg-purple-500", "bg-pink-500",
    "bg-white", "bg-black",
  ];
  ...
  const handleClearFilters = () => {
    setTempCategory(""); setTempColor(""); setTempSize("");
    setTempStyle(""); setTempPrice(500);
    setAppliedFilters({ category: "", color: "", size: "", style: "", price: 500 });
    setCurrentPage(1);
  };
  ...
  const SidebarContent = () => (
    <div className="flex flex-col gap-6">
      ...
      <div className={styles.sidebarSection}>
        <button onClick={() => toggleSection("price")} className={styles.sectionHeader}>
          Price <ChevronDown size={18} className={openSections.price ? "rotate-180" : ""} />
        </button>
        {openSections.price && (
          <div className="px-2">
            <div className="relative w-full h-6 flex items-center">
              <input
                type="range" min="50" max="500" value={tempPrice}
      ...
      <div className={styles.sidebarSection}>
        <button onClick={() => toggleSection("colors")} className={styles.sectionHeader}>
          Colors <ChevronDown size={18} className={openSections.colors ? "rotate-180" : ""} />
        </button>
        {openSections.colors && (
          <div className={styles.colorGrid}>
            {colors.map((c) => (
              ...
              >
                {tempColor === c && (
                  <Check size={14} className={c === "bg-white" ? "text-black" : "text-white"} />
                )}
              </button>
            ))}
          </div>
      ...
      <div className={styles.sidebarSection}>
        <button onClick={() => toggleSection("size")} className={styles.sectionHeader}>
          Size <ChevronDown size={18} className={openSections.size ? "rotate-180" : ""} />
        </button>
        {openSections.size && (
            ...
            {["Small", "Medium", "Large", "X-Large"].map((s) => (
              <button
                ...
                className={`${styles.sizeButton} ${
                  tempSize === s ? "bg-black text-white" : "bg-[#F0F0F0] text-black/60 hover:bg-black/10"
                }`}
              >
                {s}
              </button>
      ...
      <div className="pb-6">
        <button onClick={() => toggleSection("style")} className={styles.sectionHeader}>
          Dress Style <ChevronDown size={18} className={openSections.style ? "rotate-180" : ""} />
        </button>
        {openSections.style && (
  ...
  return (
    <div className={styles.pageWrapper}>
      ...
      />
      <div className={`${styles.mobileDrawerContent} ${isMobileFiltersOpen ? "translate-y-0" : "translate-y-full"}`}>
        <SidebarContent />
      </div>
            ...
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex items-center gap-4">
                <span className="text-black/60 hidden md:block text-sm">
                  ...
                  {Math.min(startIndex + productsPerPage, filteredProducts.length)} of {filteredProducts.length} Products
                </span>
                <button
            ...
            <div className="flex-1">
              {filteredProducts.length > 0 ? (
                <div className={styles.productGrid}>
                  {paginatedProducts.map((p) => (
                    ...
                    >
                      ...
                      <h3 className="font-bold text-sm md:text-base mb-1 truncate">{p.title}</h3>
                      <div className="flex items-center gap-1 text-yellow-400 mb-2">
                        ...
                        <span className="text-black/60 text-xs ml-1">{p.rating}/5</span>
                      </div>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={styles.priceText}>${p.price}</span>
                        {p.oldPrice && <span className={styles.oldPriceText}>${p.oldPrice}</span>}
                ...
                <div className="py-20 text-center border-b">
                  <p className="text-black/50 text-xl font-medium">No products match your filters.</p>
                </div>
              )}
            </div>

            <div className={`${styles.paginationWrapper} ${filteredProducts.length === 0 ? "opacity-30 pointer-events-none" : ""}`}>
              <button
