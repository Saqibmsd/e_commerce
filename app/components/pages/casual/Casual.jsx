...
export default function CasualPage() {
  ...
  const [tempCategory, setTempCategory] = useState("T-shirts");
  const [tempColor, setTempColor] = useState("");
  const [tempSize, setTempSize] = useState("Large");
  ...
  const SidebarContent = () => (
    <div className="flex flex-col gap-6">
      ...
      <button
        ...
        className="w-full bg-black text-white py-4 rounded-full font-medium mt-2 hover:bg-black/90 active:scale-[0.98] transition-all"
      >
        Apply Filter
      </button>
    </div>
        ...
        <div className="flex gap-8">
          ...
          <main className="flex-1">
            ...
            {filteredProducts.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 border-b pb-12">
                {filteredProducts.map((p) => (
                  <Link key={p.id} href={`/tshirt/${p.id}-${p.title.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'')}`} className="group cursor-pointer">
            ...
            <div className="flex justify-between items-center mt-10">
              <button className="px-4 py-2 border rounded-lg text-sm font-medium hover:bg-black hover:text-white transition-all">
                Previous
              </button>
